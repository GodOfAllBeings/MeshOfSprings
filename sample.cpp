...
GetDerivs( struct state state[NUMNODES], struct derivatives derivs[NUMNODES] )
{
	for( int node = 0; node < NUMNODES; node++ )
	{
		...
		sumfx += force* xm; //negative downward force - upward force - generated by stretch of spring between this and prev node
		sumfy += force* ym;
		...
		if( node < NUMNODES-1 )
		{
			...
			float stretch = length - LENGTH0;
			float force = K * stretch;
			sumfx += force * xp; //possitive downward force - downward force - generated by stretch of spring between this and next node
			sumfy += force * yp;
		}
